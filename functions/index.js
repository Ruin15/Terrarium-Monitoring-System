...
async function sendAlertEmail(userEmail, alert, ecosystem) {
  try {
    ...
    const subject = alert.severity === "critical" ?
      `🚨 CRITICAL ALERT: ${alert.type} issue in ${ecosystem} terrarium` :
      `⚠️ Warning: ${alert.type} issue in your ${ecosystem} terrarium`;
...
exports.generateSensorAlerts = onValueUpdated(
    ...
    async (event) => {
      ...
      try {
        ...
        if (!uid) {
          console.warn(
              "⚠️ No UID found in sensor data, using default thresholds");
        }
        ...
        if (uid) {
          try {
            ...
            if (profileDoc.exists) {
              ...
              console.log(
                  `✓ Using ecosystem thresholds for user ${uid}: ${ecosystem}`);
            } else {
              console.warn(
                  `⚠️ Profile not found for UID: ${uid}, using default`);
            }
          } catch (err) {
        ...
        if (!recentAlertsQuery.empty) {
          console.log(
              `⏱️ Rate limit: Alert already posted within hour for ${uid}`);
          return null;
        }
        ...
        console.log(
            `✅ Rate limit passed: ${dailyAlertsQuery.size}/5 alerts today`);
        ...
        if (alerts.length > 0) {
          ...
          await batch.commit();
          console.log(
              `✓ Stored ${alerts.length} alert(s) to Firestore for ` +
              `${ecosystem} ecosystem`);
            ...
            try {
              ...
              if (profileDoc.exists) {
                ...
                if (userEmail) {
                  ...
                  console.log(
                      `✉️ Sent ${criticalAlerts.length} critical alert ` +
                      `email(s) to ${userEmail}`,
                  );
                } else {
                  console.warn(`⚠️ No email found in profile for user ${uid}`);
                }
              } else {
                console.warn(`⚠️ Profile document not found for user ${uid}`);
              }
            } catch (emailError) {
          ...
          console.log(`ℹ️ No alerts triggered for current sensor readings`);
        }
